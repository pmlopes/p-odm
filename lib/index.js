...
var ODM = {
   ...
   *
   * @return {{findOne: Function, FindById: Function}}
   */
  view: function (modelClass, mappings) {
    var keys = Object.keys(mappings);

    return {
      findOne: function (query, callback) {
        modelClass.findOne(query, function (error, model) {
          if (error) {
          ...
          for (i = 0; i < keys.length; i++) {
            var key = keys[i];
            mappings[key].loadDbRef(model[key], stitch(key));
          }
        });
      },
      findById: function (id, callback) {
        modelClass.findById(id, function (error, model) {
          if (error) {
            return callback(error);
          }
