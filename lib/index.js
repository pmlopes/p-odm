    ...
    schemas.forEach(function (schema) {
      ...
      for (propertyName in schema.properties) {
        if (schema.properties.hasOwnProperty(propertyName)) {
          ...
          if (property.$ref) {
            if (property.$ref === 'MongoDb#ObjectId') {
              if (property.description) {
                ...
                records += '| <f' + fieldNumber + '> ' + propertyName;
                continue;
              }
            } else {
              refNum++;
              // add references
              references += '"' + collectionName + '":f' + fieldNumber + ' -> "' + property.$ref + '":f0 [id = ' + refNum + '];\n';
              // add fields to record
              records += '| <f' + fieldNumber + '> ' + propertyName;
              continue;
            }
          ...
          if (property.type === 'array') {
            ...
            if (items.$ref) {
              if (items.$ref === 'MongoDb#ObjectId') {
                ...
                refNum++;
                // add references
                references += '"' + collectionName + '":f' + fieldNumber + ' -> "' + items.$ref + '":f0 [id = ' + refNum + '];\n';
                // add fields to record
                records += '| <f' + fieldNumber + '> ' + propertyName;
                continue;
              }
          ...
          if (property.type !== 'boolean' && property.type !== 'integer' && property.type !== 'number' && property.type !== 'string' && property.type !== 'date') {
