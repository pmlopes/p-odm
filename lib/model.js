...
function isCached(cache, field, value, isModel) {
  if (cache !== undefined) {
    if (value instanceof ObjectID) {
      return cache.get(field + ':' + value.toHexString() + (isModel ? ':model' : ''));
    }
...
function putToCache(cache, field, value, isModel, doc) {
  if (cache !== undefined) {
    if (value !== undefined && value !== null) {
      if (value instanceof ObjectID) {
        cache.set(field + ':' + value.toHexString() + (isModel ? ':model' : ''), doc);
      }
    }
...
module.exports = function (mongoCollection, l2cache, schema) {
  ...
  Model.findById = function (id, fields, options, callback) {
    ...
    var cachedDocument = isCached(cache, '_id', _id, directObject);
    ...
    odm.findOne(mongoCollection, {_id: _id}, fields, options, function (err, documentLoaded) {
      ...
      if (documentLoaded === null) {
        putToCache(cache, '_id', _id, directObject, documentLoaded);
      ...
      var model = new Model(documentLoaded, { deserialize: true });
      putToCache(cache, '_id', _id, directObject, model);
      callback(null, model);
    });
  };
      ...
      if (ids.length === 0) {
        ...
        this.find({_id: {'$in': idsToFind}}, fields, options, function (err, models) {
          ...
          for (i = 0; i < models.length; i++) {
            putToCache(cache, '_id', models[i]._id, directObject, models[i]);
    ...
    if (indexFields.length === 1) {
      ...
      if (field.indexOf('.') === -1) {
        ...
        Model[methodName] = function (id, fields, options, callback) {
          ...
          var cachedDocument = isCached(cache, field, _id, directObject);
          ...
          odm.findOne(mongoCollection, query, fields, options, function (err, documentLoaded) {
            ...
            // if we search for an Id and get null it should return right away
            if (documentLoaded === null) {
              putToCache(cache, field, _id, directObject, null);
              if (includeNotFound) {
                return callback(null, null);
              } else {
                return callback(mongoCollection + ' ' + _id + ' not found');
            ...
            // special case (return direct document from mongoDB)
            if (directObject) {
              putToCache(cache, field, _id, directObject, documentLoaded);
              return callback(null, documentLoaded);
            }

            var model = new Model(documentLoaded, { deserialize: true });
            putToCache(cache, field, _id, directObject, model);
            callback(null, model);
          });
        };
  ...
  Model.prototype.reload = function (callback) {
    ...
    odm.findOne(mongoCollection, {_id: _id}, {}, {}, function (err, documentLoaded) {
      ...
      putToCache(cache, '_id', _id, true, self);
      callback(null);
    });
  };
