...
var submodelproto = {
  toJSON: function () {
    var cloned = util.clone(this._internalDocument);
    if (cloned === undefined) {
      return {};
    }
    return cloned;
          ...
          value: function (id, options, callback) {
            ...
            for (i = 0; i < value.length; i++) {
              ...
              if (el.hasOwnProperty('_id') && _id.equals(el._id)) {
                if (directObject) {
                  return callback(null, value[i], i);
                }

                return callback(null, value.get(i), i);
              }
            }
            // not found
            if (includeNotFound) {
  ...
  for (subKey in arrayInternalValue) {
    if (arrayInternalValue.hasOwnProperty(subKey)) {
      ...
      if (arrayInternalValue[subKey].$set) {
        ...
        if (value._internalDocument !== undefined) {
          val = baseArraySetter(arrayInternalValue[subKey], value._internalDocument[subKey]);
        } else {
...
module.exports = function (mongoCollection, l2cache, schema) {
  ...
  Model.prototype.save = function (options, callback) {
    ...
    odm.save(mongoCollection, self._internalDocument, options, function (err, savedDocument) {
      if (err) {
        return callback(err);
      }
      // only inserts have savedDocument
      if (self._internalDocument._id === undefined) {
        if (savedDocument) {
