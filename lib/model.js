 ...
 */
function __initialize(schema, dbdata, collectionName) {
  var data, value;
    ...
    if(dbdata.hasOwnProperty(key)) {
      ...
      if (schema.hasOwnProperty(key)) {
        ...
        if(schema[key].$set !== undefined) {
          ...
          value = __initialize(schema[key], dbdata[key], collectionName);
        }
        ...
        console.warn(key + ' is not defined in the model [' + collectionName + ']');
      }
    }
  }
...
function __buildValidator(schema, model) {
  ...
  for(var key in schema) {
    if(schema.hasOwnProperty(key)) {
      if(schema[key].$get !== undefined && schema[key].$set !== undefined) {
 ...
 * @private
 */
function _baseArraySetter(arrayInternalValue, value) {
  var obj;
  if(value) {
    obj = {};
    for (var subKey in arrayInternalValue) {
      if (arrayInternalValue.hasOwnProperty(subKey)) {
        ...
        if(arrayInternalValue[subKey].$set) {
          ...
          val = _baseArraySetter(arrayInternalValue[subKey], value[subKey]);
        }
        if (val !== undefined) {
          obj[subKey] = val;
        }
      ...
      if (value) {
        if(Array.isArray(value)) {
          ...
          if (arrayInternalValue.$set !== undefined) {
            ...
            // embedded doc types
            for (i = 0; i < value.length; i++) {
              retValue[i] = _baseArraySetter(arrayInternalValue, value[i]);
            }
          }
        } else {
          // TODO: this is wrong we should use the callbacks
    ...
    internalValue.$push = function (array, values) {
      ...
      for(var i=0; i<values.length; i++) {
        if (arrayInternalValue.$set !== undefined) {
          ...
          // embedded doc types
          retValue = _baseArraySetter(arrayInternalValue, values[i]);
        }
        Array.prototype.push.call(array, retValue);
      }
    };
  } else if (type === String) {
    internalValue.$set = _baseSetter(key, 'string', type);
  } else if (type === Number) {
    internalValue.$set = _baseSetter(key, 'number', type);
  } else if (type === Boolean) {
    internalValue.$set = _baseSetter(key, 'boolean', type);
  } else if (type === Date) {
    internalValue.$set = _baseSetter(key, 'object', type);
  ...
  var Model = function (values, options) {
    ...
    if(values !== undefined) {
      if (options && options.deserialize) {
        ...
        internalDocument = __initialize(internalSchema, values, mongoCollection);
      }
    }
      ...
      try {
        ...
        if(l2cache) {
          ...
          if(cachedDocument !== undefined) {
            ...
            // if we search for an Id and get null it should be an error
            if (cachedDocument === null) return callback(_id.toHexString() + ' not found');
        ...
        odm.collection(mongoCollection, options, function (err, collection) {
          ...
          collection.findOne({_id: _id}, options, function (err, documentLoaded) {
            ...
            // if we search for an Id and get null it should be an error
            if (documentLoaded === null) return callback(_id.toHexString() + ' not found');
