...
module.exports = function (mongoCollection, schemaDef) {
   ...
   * @memberOf Model
   */
  Object.defineProperty(Model, '$schema', {value: odm.schema.createSchema(schemaDef)});
  ...
  Model.prototype.validate = function (verbose) {
    if (Model.$schema !== undefined) {
      var validation = odm.schema.validate(this, Model.$schema);
    ...
    for (protokey in type.prototype) {
      if (protokey !== 'save' && protokey !== 'update' && protokey !== 'insert' && protokey !== 'remove' && protokey !== 'reload' && protokey !== 'validate') {
        if (type.prototype.hasOwnProperty(protokey)) {
          path = (path.indexOf('.') >= 0) ? [path].split('.') : [];
          var i;
